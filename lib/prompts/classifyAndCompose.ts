...
1. 从用户的话里抽取出每一条独立的"担心"，按以下三类归类：
   ...
   - "real"：一个**名词性的、具体的任务或物件** —— 文件、邮件、电话、文档、对话、约会等可以"打开"或"开始做"的东西。
     ✗ 错："课上不进去"（这是状态描述，不是任务）
     ✗ 错："项目推不动"（这是状态描述，不是任务）
     ✗ 错："感觉自己跟不上"（这是体验，不是任务）
     ✓ 对："导师邮件还没回"
     ✓ 对："PPT 第三页还没做"
     ✓ 对："X 课的复习笔记还没打开"
     **如果原话只有状态描述、提取不出具体物件 → 归 fog，不要硬装 real。**

   - "catastrophic"：灾难化想象（"投了一定被拒"、"面试一定砸"、"不知道是不是抑郁了"、"会被同学瞧不起"）

   - "fog"：说不清的模糊不适（"感觉很烦"、"不想动"、"就是不想做"、"提不起劲"）

2. **自我评价 / 情绪 commentary 不归入任何 bucket，直接丢弃。**
   下面这些**不是担心**，不要装进 worries：
   ✗ "今天又过去了"
   ✗ "已经动不了了"
   ✗ "我又拖了"
   ✗ "什么都做不了"
   ✗ "我没用 / 废物"
   ✗ "窗口期还有 X 天"（除非用户把它说成具体担心，否则只是事实陈述）
   只有"对未来或当下事情的判断"才进 buckets。把这些 commentary 放进 fog 会污染 fog 类、稀释信号。

3. 给「real」堆里最优先的一条，生成一个"现在 5 分钟就能开始的具体第一步"。
4. 给整张卡片生成一个 ≤15 字的温柔标题。
